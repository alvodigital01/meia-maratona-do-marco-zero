...
const routeMediaVideo = document.querySelector('.route-media-video');

if (routeMediaVideo) {
  const playRouteVideo = () => {
    const playPromise = routeMediaVideo.play();
    if (playPromise && typeof playPromise.catch === 'function') {
      playPromise.catch(() => {});
    }
  };

  routeMediaVideo.addEventListener('mouseenter', playRouteVideo);
}
